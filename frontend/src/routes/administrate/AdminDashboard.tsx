...
const AdminDashboard = () => {
  ...
  useEffect(() => {
    Axios.get("http://127.0.0.1:8000/user", {
      ...
      .then((res) => {
        ...
        if (res.data.user_role !== 3) window.location.href = "/";
  ...
  const approveProduct = (id: number) => {
    ...
    form.append("approval_status", "1");
    Axios.post("http://127.0.0.1:8000/admin1/products", form)
      .then((response) => {
  ...
  const delistProduct = (id: number) => {
    let form = new FormData();
    form.append("token", token);
    form.append("product_id", id.toString());
    form.append("approval_status", "0");
    Axios.post("http://127.0.0.1:8000/admin1/products", form)
      .then((response) => {
        shouldRefresh ? setShouldRefresh(false) : setShouldRefresh(true);
      })
  ...
  const blockSeller = (id: number) => {
    let form = new FormData();
    form.append("token", token);
    form.append("user_id", id.toString());
    form.append("attribute", "user_role");
    form.append("value", "-2");
    Axios.post("http://127.0.0.1:8000/admin1/users", form)
      .then((response) => {
        shouldRefresh ? setShouldRefresh(false) : setShouldRefresh(true);
      })
      .catch((error) => {
    ...
    Axios.get("http://127.0.0.1:8000/admin1/users/list", {
      ...
      .then((response) => {
        ...
        const sellers = response.data.users.filter(
          (user) => user.user_role === 2 || user.user_role === -2
        );
